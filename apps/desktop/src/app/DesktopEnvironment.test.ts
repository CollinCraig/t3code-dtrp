...
describe("DesktopEnvironment", () => {
  ...
  it.effect("keeps fork state separate from upstream T3 by default", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment();

      assert.equal(environment.baseDir, "/Users/alice/.dtrp-t3");
      assert.equal(environment.userDataDirName, "dtrp-t3");
    }),
  );
