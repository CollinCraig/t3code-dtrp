...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const pushCurrentBranch: GitVcsDriver.GitVcsDriver["Service"]["pushCurrentBranch"] = Effect.fn(
    ...
    if (currentUpstream) {
      const publishBranch = yield* resolvePublishBranchName(cwd, branch);
      const upstreamDefaultBranch = yield* resolveDefaultBranchName(
        cwd,
        currentUpstream.remoteName,
      ).pipe(Effect.orElseSucceed(() => null));
      const tracksDefaultBranch =
        !details.isDefaultBranch &&
        (currentUpstream.branchName === upstreamDefaultBranch ||
          currentUpstream.branchName === "main" ||
          currentUpstream.branchName === "master");
      if (currentUpstream.branchName !== publishBranch && tracksDefaultBranch) {
