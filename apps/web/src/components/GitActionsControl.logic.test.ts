...
describe("resolveLiveThreadBranchUpdate", () => {
  ...
  it("does not regress a semantic thread ref back to a temporary worktree ref", () => {
    const update = resolveLiveThreadBranchUpdate({
      threadBranch: "feature/sol/github-query-rate-limit",
      gitStatus: status({ refName: "feature/sol/bda76797" }),
    });
  ...
  it("allows a temporary worktree ref to reconcile to a semantic branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      threadBranch: "feature/sol/a9628676",
      gitStatus: status({ refName: "feature/sol/diff-panel-toggle" }),
    });

    assert.deepEqual(update, { branch: "feature/sol/diff-panel-toggle" });
  });
});
